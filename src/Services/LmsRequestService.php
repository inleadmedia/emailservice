...
class LmsRequestService {
  ...
  public function lmsRequest(string $nid, string $alias, $item_url) {
    ...
    foreach ($filteredCategories as $category) {
      $query = "/search?query=(($category->field_types_cql_query_value) AND ($category->cql_query)) AND term.acSource=\"bibliotekskatalog\" AND holdingsitem.accessionDate>=\"NOW-7DAYS\"&step=15&_source=emailservice";
